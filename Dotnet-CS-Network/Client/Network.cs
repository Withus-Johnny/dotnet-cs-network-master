...
namespace Client
{
	public static class Network
	{
		...
		public static long TimeOutTime, TimeConnected, RetryTime = NetworkController.Time + 3000;
		...
		private static void Connection(IAsyncResult result)
		{
			...
			try
			{
				...
				TimeOutTime = NetworkController.Time + 3000;
				TimeConnected = NetworkController.Time;
		...
		private static void ReceiveData(IAsyncResult result)
		{
			...
			NetworkController.BytesReceived += data.Count;
		...
		public static void Disconnect(string msg)
		{
			...
			NetworkController.Instance.isRunning = false;
		...
		public static void Process()
		{
			try
			{
				if (_client == null)
				{
					if (Connected)
					{
						while (_receiveList != null && !_receiveList.IsEmpty)
						{
							...
							if (!(p is ServerPackets.Disconnect)) continue;

							NetworkController.Instance.ProcessPacket(p);
							_receiveList = null;
							return;
						}
				...
				if (NetworkController.Time > TimeOutTime && _sendList != null && _sendList.IsEmpty)
				{
					Console.WriteLine($"C.KeepAlive");
					Enqueue(new C.KeepAlive() { Time = NetworkController.Time });
				}

				if (_sendList == null || _sendList.IsEmpty) return;

				TimeOutTime = NetworkController.Time + 2000;
				...
				NetworkController.BytesSent += data.Count;

				BeginSend(data);
			}
			catch (Exception e)
			{
				Disconnect($"연결이 해제되었습니다. Error\n{e.Message}");
			};
		}
